#ifndef OMX_NU_DEBUG__H
#define OMX_NU_DEBUG__H

#include <ctype.h>
#include <utils/Log.h>
/* 
    The OMX NU Debug interface allows debug messages to be classified by
    ...
    Features of the NU Debug interface:
...
#define OMX_DBG_DOM_VPU    0x100000ul    /* VPU communication/transfer */
...
/* Use this macro to copy a OMX_NU_Debug config received. */
#define OMX_DBG_SETCONFIG(dbg, pConfig) \
    do { \
        struct OMX_NU_Debug *pConfDbg = (struct OMX_NU_Debug *) pConfig; \
        (dbg).mask = pConfDbg->mask; \
        (dbg).out = pConfDbg->out; \
        (dbg).err = pConfDbg->err; \
    } while (0)

/* Use this macro to copy a OMX_NU_Debug config to be sent. */
...
/*
 *  GENERIC OMX NU DEBUG STATEMENT
 *
...
/*
 *  GENERIC OMX NU DEBUG Shortcut macros
 */
...
#define OMX_PRVPU0(dbg, format, list...) OMXDBG_PRINT((dbg).out, VPU, 0, (dbg).mask, format, ##list)
#define OMX_PRVPU1(dbg, format, list...) OMXDBG_PRINT((dbg).out, VPU, 1, (dbg).mask, format, ##list)
#define OMX_PRVPU2(dbg, format, list...) OMXDBG_PRINT((dbg).out, VPU, 2, (dbg).mask, format, ##list)
#define OMX_PRVPU3(dbg, format, list...) OMXDBG_PRINT((dbg).out, VPU, 3, (dbg).mask, format, ##list)
#define OMX_PRVPU4(dbg, format, list...) OMXDBG_PRINT((dbg).OMX_DBG_LEVEL4, VPU, 4, (dbg).mask, format, ##list)
#define OMX_PRVPU5(dbg, format, list...) OMXDBG_PRINT((dbg).err, VPU, 5, (dbg).mask, format, ##list)
 ...
 */
    ...
    #define OMX_DBG_DOM_VPU_STR    "VPU"
...
#endif
#ifndef OMX_DBG_COL_VPU
#define OMX_DBG_COL_VPU OMX_DBG_COL_CYAN
#endif
