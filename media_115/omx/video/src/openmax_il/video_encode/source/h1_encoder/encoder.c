...
#endif
#include <utils/Log.h> /*a@nufront*/
#include <utils/nufrontlog.h> /*a@nufront*/
#ifdef ENCH1
...
#ifndef ENCODER_TRACE
static int fprintf_dummy(FILE* s, const char* f, ...) { return 0; }
#endif
...
OMX_U32 encoder_thread_main(BASECOMP* base, OMX_PTR arg)
{
    ...
    if (err != OMX_ErrorNone)
    {
        ...
        LDCLOGD("ASYNC: new state: %s,line=%d", HantroOmx_str_omx_state(OMX_StateInvalid),__LINE__);
                                      ...
                                      OMX_IN    OMX_U32 nSizeBytes)
{
    ...
    *ppBuffer = buff->header;
    ...
    LDCLOGD("API: data (virtual address): 0x%08x pBufferHeader: 0x%08x\n", bus_data, *ppBuffer);
    LDCLOGD("exit");
                                  ...
                                  OMX_IN OMX_BUFFERHEADERTYPE* pBufferHeader)
{
    ...
    {
        LDCLOGD("OMX_ErrorPortUnpopulated, line=%d",__LINE__);
                                    ...
                                    OMX_IN OMX_PTR        pParam)
{
    ...
    switch (nIndex)
    {
        ...
        case OMX_IndexParamPortDefinition:
            {
                ...
                switch (param->nPortIndex)
                {
                    case PORT_INDEX_INPUT:
                        ...
                        port->nBufferSize                     = param->nBufferSize; /*a@nufront*/
                        LDCLOGD("port->nBufferSize = %d w=%d,h=%d",port->nBufferSize,port->format.video.nFrameWidth,port->format.video.nFrameHeight );
                        LDCLOGD("port->format.video.nStride = %d ",port->format.video.nStride);
                        break;
                    ...
                    case PORT_INDEX_OUTPUT:
                        ...
                        LDCLOGD("PORT_INDEX_OUTPUT line=%d",__LINE__);
       ...
       case OMX_IndexParamCompBufferSupplier:
        ...
        {
            LDCLOGD("OMX_IndexParamVideoBitrate line= %d",__LINE__);
            ...
            if ( paramBitrate->nPortIndex == PORT_INDEX_OUTPUT )
            {
                ...
                {
                    LDCLOGD("OMX_EventPortSettingsChanged");
...

{
    ...
    switch (nIndex)
    {
        ...
        case OMX_IndexConfigCommonInputCrop:
            {
                ...
                calculate_new_outputBufferSize( pEnc );
                LDCLOGD(" calculate_new_outputBufferSize line = %d",__LINE__);
                pEnc->app_callbacks.EventHandler(
                    pEnc->self,
                    pEnc->app_data,
                    OMX_EventPortSettingsChanged,
        ...
        case OMX_IndexConfigVideoBitrate:
            {
                LDCLOGD("OMX_IndexConfigVideoBitrate line=%d",__LINE__);
        ...
        case OMX_IndexConfigVideoFramerate:
            {
                LDCLOGD("OMX_IndexConfigVideoFramerate line=%d",__LINE__);
...
OMX_ERRORTYPE OMX_ComponentInit( OMX_HANDLETYPE hComponent )
{
...
#endif
    OMX_ERRORTYPE err = HantroOmx_port_init(&pEnc->inputPort, 4, 4, 5, 64*8*2);
    if (err != OMX_ErrorNone)
        goto INIT_FAILURE;
    ...
    if ( ( ((640+15)/16 )*((480+15)/16) ) > 1620 )
        outputBufferSize = (176*144*3/2/8) * 2;
    else
        outputBufferSize = (176*144*3/2/4) * 2;
#else
    outputBufferSize = (176*144*3/2/4) * 2 * 20;
#endif
    err = HantroOmx_port_init(&pEnc->outputPort, 4, 4, 5, outputBufferSize );
    if (err != OMX_ErrorNone)
        goto INIT_FAILURE;
    ...
    pEnc->inputPort.def.format.video.eColorFormat       =  OMX_COLOR_FormatYUV420SemiPlanar; /*m@nufront*/
...
OMX_ERRORTYPE set_avc_defaults( OMX_ENCODER* pEnc)
{
    ...
    /*a@nufront: fix bug2712: lower quantization value, reducing the block effect*/
...
OMX_ERRORTYPE set_vp8_defaults( OMX_ENCODER* pEnc )
{
    ...
    vp8Ref->nPreviousFrameRefresh = OMX_TRUE;
 ...
 */
 ...
 static OMX_ERRORTYPE calculate_new_outputBufferSize( OMX_ENCODER* pEnc )
 {
    ...
    LDCLOGD(" calculate_new_outputBufferSize  OMX_EventPortSettingsChanged portDef->nBufferSize = %d ,line=%d",portDef->nBufferSize ,__LINE__);
                                             ...
                                             OMX_PTR pCmdData, OMX_PTR arg)
{
    ...
    if (nextstate == pEnc->state)
    {
        ...
        LDCLOGD("ASYNC: same state:%s line=%d\n", HantroOmx_str_omx_state(pEnc->state),__LINE__);
    ...
    if (err != OMX_ErrorNone)
    {
        ...
        LDCLOGD("ASYNC: set state error:%s line=%d\n", HantroOmx_str_omx_err(err),__LINE__);
...
OMX_ERRORTYPE async_encoder_flush_port(OMX_COMMANDTYPE Cmd, OMX_U32 nParam1, OMX_PTR pCmdData, OMX_PTR arg)
{
 ...
 FAIL:
    ...
    LDCLOGD("ASYNC: error while flushing port: %s line=%d\n", HantroOmx_str_omx_err(err),__LINE__);
    pEnc->state = OMX_StateInvalid;
    pEnc->run   = OMX_FALSE;
    pEnc->app_callbacks.EventHandler(pEnc->self, pEnc->app_data, OMX_EventError, err, 0, NULL);
    return err;
...
OMX_ERRORTYPE async_encoder_mark_buffer(OMX_COMMANDTYPE Cmd, OMX_U32 nParam1, OMX_PTR pCmdData, OMX_PTR arg)
{
    ...
    else
    {
        ...
        LDCLOGD("ASYNC: no space for mark line=%d",__LINE__);
...
OMX_ERRORTYPE async_start_stream( OMX_ENCODER* pEnc )
{
    ...
    int retry = 4000;
    while (outputBuffer == NULL && retry > 0 && pEnc->state != OMX_StatePause && pEnc->statetrans != OMX_StatePause)
    {
            TRACE_PRINT("ASYNC: %s,retry=%d\n", __FUNCTION__,retry);
            LDCLOGD("ASYNC: retry=%d\n",retry);
            if((pEnc->state == OMX_StateExecuting)&&(pEnc->statetrans == OMX_StateIdle))
            {
                    LDCLOGD("exit  line = %d",__LINE__);
    ...
    if (outputBuffer == NULL)
    {
        ...
        LDCLOGD("ASYNC: there's no output buffer!\n");
    ...
    outputBuffer->header->nFlags = OMX_BUFFERFLAG_CODECCONFIG; /*a@nufront*/
...
OMX_ERRORTYPE async_video_encoder_encode(OMX_ENCODER* pEnc)
{
    ...
    {
        LDCLOGD("ldc: ASYNC: call < async_start_stream >\n");
        ...
        if (err != OMX_ErrorNone)
        {
            if (err == OMX_ErrorOverflow)
            {
                ...
                LDCLOGD("ASYNC: firing OMX_EventErrorOverflow line=%d",__LINE__);
    ...
    camera_buf *pCamera = (camera_buf *)(inputBuffer->header->pBuffer);
    ...
    if ( inputBuffer->header->nOffset != 0 ||
         ...
         pCamera->size < frameSize  ) /*m@nufront*/
         //inputBuffer->header->nOffset < frameSize  )
    {
        LDCLOGD("ASYNC: Using internal buffer");
        ...
        assert(tempBuffer->capacity - tempBuffer->size >= pCamera->size);
#if 0
...
#else
        OMX_U32 len = pCamera->size;
        OMX_U8* src = pCamera->vir_addr + inputBuffer->header->nOffset;
        OMX_U8* dst = tempBuffer->bus_data + tempBuffer->size; // append to the buffer
#endif
        memcpy(dst, src, len);
        tempBuffer->size += len;
            ...
            if ( tempBuffer->size >= frameSize *2 )
            {
                ...
                if (err != OMX_ErrorNone)
                {
                    ...
                    else if (err == OMX_ErrorBadParameter)
                    {
                        ...
                        LDCLOGD("ASYNC: firing OMX_ErrorBadParameter line=%d",__LINE__);
        ...
        else if( tempBuffer->size >= frameSize )
        {
            ...
            if (err != OMX_ErrorNone)
            {
                ...
                {
                    TRACE_PRINT(pEnc->log, "ASYNC: firing OMX_EventErrorOverflow\n");
                    LDCLOGD("ASYNC: firing OMX_EventErrorOverflow line=%d\n",__LINE__);
                    pEnc->app_callbacks.EventHandler(pEnc->self, pEnc->app_data, OMX_EventError, OMX_ErrorOverflow, 0, NULL);
                }
                else if (err == OMX_ErrorBadParameter)
                {
    ...
    {
        LDCLOGD("ASYNC: No internal buffer\n");
...
#else
        OMX_U8* bus_data = pCamera->vir_addr;
        OSAL_BUS_WIDTH bus_address = pCamera->phy_addr;
        OMX_U32 filledlen = pCamera->size;
#endif
        OMX_U32 retlen    = 0;
 ...
 INVALID_STATE:
    assert(err != OMX_ErrorNone);
    /*a@nufront*/
    if(err == OMX_ErrorVendorStartUnused)
       return OMX_ErrorNone;

    TRACE_PRINT(pEnc->log, "ASYNC: error while processing buffers: %s\n", HantroOmx_str_omx_err(err));
    LDCLOGD("ASYNC: error while processing buffers: %s,line=%d", HantroOmx_str_omx_err(err),__LINE__);
            ...
            case OMX_VIDEO_CodingVP8:
                {
                    ...
                    frame.bPrevRefresh = pEnc->encConfig.vp8Ref.nPreviousFrameRefresh;
                                ...
                                OMX_U32 datalen, BUFFER* inputBuffer, OMX_U32* retlen, OMX_U32 frameSize )
{
    ...
    while (datalen >= frameSize )
    {
        ...
        int retry = 4000;
        //while (outputBuffer == NULL && retry > 0 && dec->state != OMX_StatePause && dec->statetrans != OMX_StatePause)
        while (outputBuffer == NULL && retry > 0 && pEnc->state != OMX_StatePause && pEnc->statetrans != OMX_StatePause)
        {
                TRACE_PRINT("ASYNC: %s,retry=%d\n", __FUNCTION__,retry);
                LDCLOGD("ASYNC: %s,retry=%d\n", __FUNCTION__,retry);
                if((pEnc->state == OMX_StateExecuting)&&(pEnc->statetrans == OMX_StateIdle))
                {
                        LDCLOGD("exit %s line = %d",__FUNCTION__,__LINE__);
                        return OMX_ErrorVendorStartUnused;
                }
                OSAL_ThreadSleep(RETRY_INTERVAL);
        ...
        outputBuffer->header->nFlags     = inputBuffer->header->nFlags & ~OMX_BUFFERFLAG_EOS;
        /*a@nufront*/
        if(stream.IntraFrame == 1)
        {
              outputBuffer->header->nFlags = outputBuffer->header->nFlags | OMX_BUFFERFLAG_SYNCFRAME;
              LDCLOGD("Is IntraFrame");
        }
        TRACE_PRINT(pEnc->log, "ASYNC: output buffer OK\n");
...
OMX_ERRORTYPE async_image_encoder_encode(OMX_ENCODER* pEnc)
{
 ...
 INVALID_STATE:
    ...
    TRACE_PRINT(pEnc->log, "ASYNC: error while processing buffers: %s\n", HantroOmx_str_omx_err(err));
    LDCLOGD("ASYNC: error while processing buffers: %s,line=%d\n", HantroOmx_str_omx_err(err),__LINE__);
