...
void write_ivf_frame_header(OMX_BUFFERHEADERTYPE* buffer, OMXCLIENT * appdata)
{
    int byteCnt = 0;
    ...
    /* Timestamp (8 bytes) */
    data[4]  =  (appdata->frame_count)        & 0xff;
    data[5]  = ((appdata->frame_count) >> 8)  & 0xff;
    data[6]  = ((appdata->frame_count) >> 16) & 0xff;
    data[7]  = ((appdata->frame_count) >> 24) & 0xff;
    data[8]  = ((appdata->frame_count) >> 32) & 0xff;
    data[9]  = ((appdata->frame_count) >> 40) & 0xff;
    data[10] = ((appdata->frame_count) >> 48) & 0xff;
    data[11] = ((appdata->frame_count) >> 56) & 0xff;

    fwrite(data, sizeof(OMX_U8), IVF_FRM_BYTES, appdata->output);
}
...
OMX_STRING OMX_OSAL_TraceCodingTypeStr(OMX_IN OMX_VIDEO_CODINGTYPE coding)
{
    ...
    case OMX_VIDEO_CodingCUSTOM_1:
        return "OMX_VIDEO_CodingCUSTOM_1";
    case OMX_VIDEO_CodingCUSTOM_1_3:
        return "OMX_VIDEO_CodingCUSTOM_1_3";
    case OMX_VIDEO_CodingCUSTOM_2:
        return "OMX_VIDEO_CodingCUSTOM_2";
    case OMX_VIDEO_CodingCUSTOM_3:
        return "OMX_VIDEO_CodingCUSTOM_3";
                                                ...
                                                buffer)
{
    ...
    if((client->domain != OMX_PortDomainImage) && 
       (client->coding_type == OMX_VIDEO_CodingVP8) && buffer->nFilledLen > 0)
    {
        OMX_OSAL_Trace(OMX_OSAL_TRACE_DEBUG, "Wrote ivf frame header\n");
        write_ivf_frame_header(buffer, client);
    ...
    if((client->coding_type == OMX_VIDEO_CodingVP8) && 
       (client->domain == OMX_PortDomainVideo))
    {
        ...
        else if (intraPicRate && (client->output_buffer_count%intraPicRate == 1))
        {
                                  ...
                                  OMX_COLOR_FORMATTYPE inputMode)
{
    ...
    else if(inputMode == OMX_COLOR_Format24bitRGB888)
    {
    ...
    if(inputMode == OMX_COLOR_Format24bitRGB888)
    {
        sliceCrOffset = width * height + 
                                          ...
                                          OMX_U32 firstVop, OMX_U32 lastVop)
{
    ...
    if(appdata->coding_type == OMX_VIDEO_CodingVP8)
    {
        OMX_OSAL_Trace(OMX_OSAL_TRACE_DEBUG, "Wrote ivf file header\n");
                                           ...
                                           OMX_U32 firstVop, OMX_U32 lastVop)
{
    ...
    if(appdata->coding_type == OMX_IMAGE_CodingWEBP)
    {
