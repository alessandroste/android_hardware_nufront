...
if [ -s "test_data_parameter_vp8.sh" ] ; then
    TEST_PARAMETER=test_data_parameter_vp8.sh
else
    TEST_PARAMETER=customer_data_parameter_vp8.sh
fi
...
refencode() {
	...
	--dctPartitions=${dctPartitions[${set_nro}]} \
    --errorResilient=${errorResilient[${set_nro}]} \
	--ipolFilter=${ipolFilter[${set_nro}]} \
	--filterType=${filterType[${set_nro}]} \
...
encode() {
    ...
    case "${inputFormat[$set_nro]}" in
        0|1) {
            ...
            let bufferSize=${width[${set_nro}]}*${height[${set_nro}]}*3/2
              };;
        2|3|4|5|6|8) {
            #ycbycr | cbycry | rgb565 | bgr565 | rgb555 | rgb444
            let bufferSize=${width[${set_nro}]}*${height[${set_nro}]}*2
              };;
        10) {
            #rgb888 | bgr888 
            let bufferSize=${width[${set_nro}]}*${height[${set_nro}]}*4   
              };;
        * )
    esac
    ...
    -Y ${verOffsetSrc[${set_nro}]} \
       ${omxRateControl} \
    -L ${level[${set_nro}]} \
    -d ${dctPartitions[${set_nro}]} \
    -R ${errorResilient[${set_nro}]} \
    -a ${firstVop[${set_nro}]} \
    -b $omxLastVop \
    -V $omxnPFrames
}
...
compare() {
    if [ -s "$case_dir/output.ivf" ]; then
        if cmp $case_dir/output.ivf $case_dir/reference.ivf; then
            ...
            if [ -s "$VP8DEC" ]; then
                $VP8DEC -Oref.yuv $case_dir/reference.ivf >> ref.log
                $VP8DEC -Oomx.yuv $case_dir/output.ivf >> omx.log
        ...
        echo "Case $case_nro FAILED"
        echo "E case_$case_nro;SW/HW Integration;;$date;Failed;;$hwtag;$USER;;;$swtag;$systag"
        echo "failed, $case_dir/output.ivf size 0 or file not found"
    fi
}
