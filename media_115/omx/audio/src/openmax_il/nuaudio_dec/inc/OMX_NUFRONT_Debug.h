 ...
 *File name:     OMX_NUFRONT_Debug.h 
 *Author: Zhai Jianfeng Version: v1.0       Date: 2010-09-25
 * @file OMX_NUFRONT_Debug.h
 *
 * This file implements the NUFRONT-specific OMX Debug interface 
 *
 * @rev  0.1
*/
/* -------------------------------------------------------------------------- */
...
#ifndef OMX_NUFRONT_DEBUG__H
#define OMX_NUFRONT_DEBUG__H

#include <ctype.h>
#include <utils/Log.h>
/* 
    The OMX NUFRONT Debug interface allows debug messages to be classified by
    ...
    Features of the NUFRONT Debug interface:
...
/* Use this macro to copy a OMX_NUFRONT_Debug config received. */
#define OMX_DBG_SETCONFIG(dbg, pConfig) \
    do { \
        struct OMX_NUFRONT_Debug *pConfDbg = (struct OMX_NUFRONT_Debug *) pConfig; \
        (dbg).mask = pConfDbg->mask; \
        (dbg).out = pConfDbg->out; \
        (dbg).err = pConfDbg->err; \
    } while (0)

/* Use this macro to copy a OMX_NUFRONT_Debug config to be sent. */
...
/*
 *  GENERIC OMX NUFRONT DEBUG STATEMENT
 *
...
/*
 *  GENERIC OMX NUFRONT DEBUG Shortcut macros
 */
