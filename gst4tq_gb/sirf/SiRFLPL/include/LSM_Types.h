 ...
 *
 *  $File: //customs/customer/Marvell-U1/sirf/Software/sirf/SiRFLPL/include/LSM_Types.h $
 *
...
#define LSM_MAX_FREQ 8
...

   {
      ...
      LSM_CP_SESSION_STARTED = 0x00000040,

      /** LPL is going to resume the network connection for SUPL. */

      LSM_SUPL_NETWORK_RESUME = 0x00000080,
      
      /** LPL has closed the network connection for SUPL. */

      LSM_SUPL_NETWORK_CLOSED = 0x00000100,

      /** LPL has failed the network connection for SUPL. */

      LSM_SUPL_NETWORK_FAILED = 0x00000200,
   ...
   */
...

   {
      ...
      /** AGPS session with decoded assistance data */
      LSM_TEXT_AIDING_SESSION,
   ...
   *  @brief Aiding Capabilities CGEE/SGEE/NAVBIT */
...

   {
      ...
      LSM_RESET_TYPE_AUTO_RESET,
         
      /** for test mode 7 **/
      
      LSM_RESET_TYPE_TEST_MODE
    ...
    *  @param[out] msgLen: SSB/STAT message length. */
   typedef tSIRF_VOID (*LSM_TestModeListener)     (LSM_UINT32 msg_id, LSM_UINT8* message, LSM_UINT32 msgLen);
...

   {
      ...
      LSM_CHAR m_SerialPort[MAX_PORT_NUM_STRING_LENGTH];
...

   {
...

      {
         ...
         /** 0: disable test mode; 1: enable enable mode. */
         LSM_UINT isTestModeListenerSet: 1;
...

      {
         ...
         /** Transferred frequency nominal value. */

         LSM_DOUBLE frequencyNominalValue;
      ...
      /** message call back function for test mode **/
      LSM_TestModeListener testModeCallback;
   ...
   typedef struct
   {
      ...
      struct
      {
         ...
         /** 0: Not set, 1: SiRFAware availability set. */
         LSM_UINT isSiRFAwareSet: 1;
      }  SessionCfgBitMap;
      ...
      /** LSM_FALSE: SiRFAware not available; LSM_TRUE: SiRFAware available. */
      LSM_BOOL isSiRFAwareAvailable;
