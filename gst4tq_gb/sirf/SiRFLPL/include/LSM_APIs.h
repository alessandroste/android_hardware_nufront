 ...
 *
 *  $File: //customs/customer/Marvell-U1/sirf/Software/sirf/SiRFLPL/include/LSM_APIs.h $
 *
...
extern "C"
{
   ...
   */
   ...
   DLL_EXPORT
#endif /*DLL*/
   LSM_BOOL  LSM_setAgpsServer(tSIRF_CHAR* ipAddress, tSIRF_UINT16 portNumber);

   /**
   * @fn           LSM_BOOL LSM_Init(LSM_commonCfgData *pCommonCfgData)
   * @brief        AGPS driver/service/deamon calls this API modify the SET Capabilities and/or
   *               preferred location method.
   * @param[in]    pSetCapabilities pointer to capabilities structure; if NULL then capabilities will not be modified.
   *    
   * @return       LSM_TRUE: success; @n LSM_FALSE: failure.
   */
   ...
   DLL_EXPORT
#endif /*DLL*/
  LSM_BOOL LSM_setCapabilities(LSM_SETcapabilities* pSetCapabilities);


#ifdef DLL
			DLL_EXPORT
#endif /*DLL*/
   LSM_BOOL	LSM_setSecure(LSM_BOOL secure);
   ...
   */
   ...
   /**
   * @fn           LSM_BOOL LSM_GetSGEEFileAge(LSM_UINT32 *pAge, LSM_UINT32 *pPrediction, LSM_UINT8 satId)
   * @brief        To get age and prediction information of SGEE file.
   * @param[in]    pAge: pointer to recieve age value (in seconds).
   * @param[in]    pPrediction: pointer to recieve prediction value (in seconds).
   * @return       LSM_TRUE: Get age values successfully.
   *            @n LSM_FALSE: Could not retrieve age values.
   */

   LSM_BOOL LSM_GetSGEEFileAge(LSM_UINT32 *pAge, LSM_UINT32 *pPrediction);

  /**
   * @fn           LSM_BOOL LSM_UpdateFrequency(LSM_HW_FREQ_UPDATE* frequency_update)
   */

   LSM_BOOL LSM_UpdateFrequency(LSM_HW_FREQ_UPDATE* frequency_update);

   /**
   * @fn           LSM_BOOL LSM_SetReferenceTime( LSM_UINT64 utcTime, LSM_UINT64 timestamp, LSM_SINT32 uncertainty)
   * @brief        To inject the reference time.
   * @param[in]    utcTime: Current Utc Time(in miliseconds).
   * @param[in]    timestamp: system timestamp (in miliseconds).
   * @param[in]    uncertainty: Uncertainty (in miliseconds).
   */
   LSM_BOOL LSM_SetReferenceTime( LSM_UINT64 utcTime, LSM_UINT64 timestamp, LSM_SINT32 uncertainty);
