 ...
 *              can only be called after CLM is started.
 *
 * @return tCLM_STATUS with CLM_SUCCESS or error code defined in tCLM_STATUS
 *
 */
...
#ifndef EMB_SIF 
/**
 * @fn tCLM_STATUS CLM_CheckValidConfig(tCLM_CONFIG *  pConfigData)
 * @brief CLM_CheckValidConfig, optional API to be called.
 *
 * @description To check the CLM configuration options (SGEE/CGEE/SGEE-CGEE
 *              mode of operation,OTA and storage file format, SGEE and CGEE
 *              age limit and debug log options are valid or Invalid
 *
 * @param[in] pConfigData Pointer to configuration structure filled with the required
 *                        CLM configuration options
 * @return tCLM_STATUS with CLM_SUCCESS or error code defined in tCLM_STATUS
 *
 */
tCLM_STATUS CLM_CheckValidConfig(
    tCLM_CONFIG *  pConfigData
    );

/**
 * @fn tCLM_STATUS CLM_GetIsSGEEDownloadInProgress(tSIRF_VOID)
 *
 * @brief CLM_GetIsSGEEDownloadInProgress
 *
 * @description To get the status of SGEE download is in progress
 *
 * @param[in] VOID
 * @return tCLM_STATUS with CLM_SUCCESS or error code defined in tCLM_STATUS
 *
 */
tCLM_STATUS CLM_GetIsSGEEDownloadInProgress( tSIRF_VOID);
...
/**
 * @fn tCLM_STATUS CLM_SendGPSTime(tCLM_GPS_TIME_INFO * pTimeInfo)
 * @brief CLM_SendGPSTime
 *
 * @description To send the current GPS time to SIF.
 *
 * @param[in] pTimeInfo Pointer to current GPS Time Info.
 * @return tCLM_STATUS with CLM_SUCCESS or error code defined in tCLM_STATUS
 *
 */
tCLM_STATUS CLM_SendGPSTime(
    tCLM_GPS_TIME_INFO * pTimeInfo
);
...
/**
 * @fn tCLM_STATUS CLM_GetCGEEPredictionInfo( tCLM_CGEE_PREDICTION_INFO *pInfo)
 * @brief Get the CGEE Prediction info.
 *
 * @description Get the CGEE Prediction info.
 *
 * @param tSIRF_VOID.
 *
 * @return tCLM_STATUS         on success CLM_SUCCESS\ on error CLM_COMMON_NOT_RUNNING
 *                                                               CLM_GENERIC_FAILURE
 *                                                               CLM_COMMON_INVALID_PARAM
 *                                                               CLM_COMMON_INVALID_OPS
 */

tCLM_STATUS CLM_GetCGEEPredictionInfo( tCLM_CGEE_PREDICTION_INFO *pInfo);

/**
 * @fn tSIRF_BOOL CLM_isStarted(tSIRF_VOID)
 * @brief checks if CLM started or not
 *
 * @description checks if CLM started or not.
 *
 * @param tSIRF_VOID.
 *
 * @return tCLM_STATUS         on success CLM_SUCCESS\ on error CLM_COMMON_NOT_RUNNING
...
tCLM_STATUS CLM_isStarted(tSIRF_VOID);

/**
 * @fn tCLM_STATUS CLM_UpdateEEDataToNVM(tSIRF_UINT32 reserved)
 * @brief CLM_UpdateEEDataToNVM (Usable only in case when CLM_CTRL_MODE_STORAGE_ON_DEMAND is set)
 *
 * @description To instruct the SIF to Write the SIF Storage data to NVM in case of 
 *              Storage Update Mode is selected as CLM_CTRL_MODE_STORAGE_ON_DEMAND in 
 *              operationMode's Bit4-7 of struct tCLM_CONFIG when calling CLM_Configure()
 *
 * @param[in] reserved Reserved for future use
 * @return tCLM_STATUS with CLM_SUCCESS or error code defined in tCLM_STATUS
 *
 */
tCLM_STATUS CLM_UpdateEEDataToNVM(
