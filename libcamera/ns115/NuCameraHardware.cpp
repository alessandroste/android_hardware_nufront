...
int NuCameraHardware::previewThread()
{
   ...
   /* if(!isPreviewTime()){
    ...
    //zhuyx--add--[Test for get raw frame from sensor
    char prop_value[10];
    char frame_number[10];
    //get property in threadloop will waste cpu time for preview/recording ,but it not affect performance.
    property_get(CAMERA_TEST_PROP, prop_value, "0");
    if(!strcmp(prop_value,"1")){
        property_get(CAMERA_TEST_FRAME_NUM, frame_number, "0");
        if(mCameraTestFrameNumber <= atoi(frame_number)){
            writeFrame(mCameraTestFrameNumber, pmem.virtualAddress, frameSize);
            mCameraTestFrameNumber++;
        }else{
            //beyond user set number, over write date to file.
            mCameraTestFrameNumber = 0;
        }
    }
    //zhuyx--add--]
...
void NuCameraHardware::stopPreview()
{
    ...
    //clear camera test frame number
    mCameraTestFrameNumber = 0;
}
...
int NuCameraHardware::autoFocusThread()
{
	...
	//zhuyx@nufront for bug:8246--[
    if (mNuCameraV4L2->getCameraId() == NuCameraV4L2::CAMERA_ID_BACK){
        LOGV("%s, Call NuCameraV4L2->setTakepicureFlag(TAKE_PICUTRE_FLAG)",__func__);
        if (mNuCameraV4L2->setTakepicureFlag(TAKE_PICUTRE_FLAG) < 0) {
            LOGE("ERR(%s):Fail on mNuCameraV4L2->setTakepicureFlag(TAKE_PICUTRE_FLAG)", __func__);
			return UNKNOWN_ERROR;
        }
        //Close flashing
...
static int sCameraNum = sizeof(sCameraInfo) / sizeof(sCameraInfo[0]);

#define  KEY_CAMERA_BACK_ORIENTATION  "android.hardware.camera-back-orientation"
#define  KEY_CAMERA_FRONT_ORIENTATION  "android.hardware.camera-front-orientation"

static void initCameraInfoGlobal(){
    LOGV("====init camera info====");
    CameraInfo *pinfo = &sCameraInfo[0];
    if (has_fdt()){
        LOGV("====init camera using FDT====");
        sCameraNum = 0;
        int back = -1, front=-1;
        char buf[256] = {0};
        fdt_get(KEY_CAMERA_BACK_ORIENTATION, buf, "");
        if (strlen(buf)>0) {
            sCameraNum++;
            back = atoi(buf);
        }
        fdt_get(KEY_CAMERA_FRONT_ORIENTATION, buf, "");
        if (strlen(buf)>0) {
            sCameraNum++;
            front = atoi(buf);
        }
        if(sCameraNum > 0){
            if (back != -1) {
                pinfo->facing = CAMERA_FACING_BACK;
                pinfo->orientation = back;
                pinfo++;
            }
            if (front != -1) {
                pinfo->facing = CAMERA_FACING_FRONT;
                pinfo->orientation = front;
            }
        }
    }
}

class InitCameraInfoGlobal {
public:
    InitCameraInfoGlobal(){
        initCameraInfoGlobal();
    }
};

/*
 *  Object 'g_dummy_camera_info' must be define later than 'sCameraInfo[]',
 *  if you modify the code, REMEMBER it.
 *
 */
static InitCameraInfoGlobal  g_dummy_camera_info;
