...
#define LOG_TAG "NuSocketClient"
#include <cutils/log.h>

#include "NuSocketClient.h"

NuSocketClient::NuSocketClient(int socket, bool owned)
        : mSocket(socket)
        , mSocketOwned(owned)
        , mPid(-1)
        , mUid(-1)
...
int NuSocketClient::sendMsg(int code, const char *msg, bool addErrno) {
    char *buf;
    const char* arg;
    const char* fmt;
    char tmp[1];
...
int NuSocketClient::sendMsg(const char *msg) {
    if (mSocket < 0) {
        errno = EHOSTUNREACH;
        return -1;
    }
...
int NuSocketClient::sendData(const void* data, int len) {
    int rc = 0;
...
bool NuSocketClient::decRef() {
    ...
    if (mRefCount == 0) {
        ...
        SLOGE("NuSocketClient refcount went negative!");
    }
    pthread_mutex_unlock(&mRefCountMutex);
    if (deleteSelf) {
        delete this;
