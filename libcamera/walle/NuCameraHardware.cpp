...
// frame skip
#define INITIAL_SKIP_FRAME  3
#define EFFECT_SKIP_FRAME   1
#define SNAP_SKIP_FRAME     2
...
//define for walle
#define CAMERA_WALLE_SRV_RUNNING "camera.walle.srv.running"
#define CAMERA_HAL_RUNNING "camera.hal.running"
#define WALLE_GESTURE_CTRL_MODE_MOUSE 1
#define WALLE_GESTURE_CTRL_MODE_KEYBOARD 2
...
#ifdef WALLE_PARAMS
#define  WALLE_SOCKET   "walle_socket"
NotificationManager *gNM = NotificationManager::getInstance(WALLE_SOCKET);

static void notify(int n){
    LOGD("---test---, call gNM->sendNotification:%d", n);
    gNM->sendNotification(n);
}
#endif
...
NuCameraHardware::NuCameraHardware(int cameraId, camera_device_t *dev)
        :
          ...
          mLastPreviewed(0),
          mPreviewAfter(0),
          mWalleMode(WALLE_3THRD_GESTURE),
          ...
          mCameraSensorName(NULL)
{
    ...
    ret = mNuCameraV4L2->initCamera(cameraId);
    if (ret < 0) {
        LOGE("ERR(%s):Fail on mNuCameraV4L2 init", __func__);
    }

    //zhuyx--add--for-uinput--[
#ifdef WALLE_PARAMS
    LOGI("INFO(%s), Call uinput_create() to init uinput device.", __func__);
    uinput_create("/dev/uinput", NULL);
    if( (fd = mNuCameraV4L2->getCameraFd()) < 0 ){
        LOGE("ERR(%s): The fd(%d) is invalid!", __func__, fd);
    }
    mBayer2NVHeap = (unsigned char *)calloc(1, 960 * 480);
    if (!mBayer2NVHeap) {
        LOGE("ERR(%s): Walle Bayer2NV heap creation fail", __func__);
    }
    LOGI("INFO(%s), Call InitHandControlProcess(fd:%d) to init gestureRecognitionPrecess.", __func__, fd);
    InitHandControlProcess(fd);
    setOperationModel(WALLE_GESTURE_CTRL_MODE_MOUSE);//mouse mode(1) as default
    LOGI("INFO(%s), Call startNotifyThread to start notify socket.", __func__);
    gNM->startNotifyThread();
#endif
    //zhuyx--add--for-uinput--]
...
inline void   NuCameraHardware::initFrontParameters()
{
    CameraParameters p;

    // Set preview & picture size supported
    ...
    p.set(CameraParameters::KEY_SUPPORTED_VIDEO_SIZES,
          "320x240,176x144");
    p.set(CameraParameters::KEY_VIDEO_SIZE, "320x240");
    p.set(CameraParameters::KEY_PREFERRED_PREVIEW_SIZE_FOR_VIDEO,"320x240");
    ...
    //Set Preview Size
    p.setPreviewSize(FRONT_PREVIEW_WIDTH, FRONT_PREVIEW_HEIGHT);
    ...
    //frame rate
    p.set(CameraParameters::KEY_SUPPORTED_PREVIEW_FRAME_RATES, FRONT_SUPPORT_FRAME_RATES);
    ...
    //focal length
    p.set(CameraParameters::KEY_FOCAL_LENGTH, FRONT_CAMERA_FOCAL_LENGTH_STR);
#ifndef WALLE_PARAMS
    //jpma@nusmart begin: add zoom support for front camera
    //ZOOM
    //KEY_ZOOM_SUPPORTED, KEY_ZOOM, KEY_MAX_ZOOM, KEY_ZOOM_RATIOS, KEY_SMOOTH_ZOOM_SUPPORTED
    //We Support 2Times ZOOM and each times up 0.1
    ...
    //jpma@nusmart end: add zoom support for front camera
#endif
    //view field
    p.set(CameraParameters::KEY_HORIZONTAL_VIEW_ANGLE, FRONT_CAMERA_HVEIW_ANGLE);
    p.set(CameraParameters::KEY_VERTICAL_VIEW_ANGLE, FRONT_CAMERA_VVEIW_ANGLE);
#ifndef WALLE_PARAMS
...
inline void   NuCameraHardware::initBackParameters()
{
    CameraParameters p;
#ifdef WALLE_PARAMS
	p.set(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES,
			"1280x480,960x480,640x480,320x240,176x144");
    p.set(CameraParameters::KEY_SUPPORTED_PICTURE_SIZES,
          "640x480,320x240");
    p.set(CameraParameters::KEY_SUPPORTED_VIDEO_SIZES,
          "320x240,176x144");
    ...
    for (int i = 0; i < mSupportedPreviewSizes.size(); i++) {
    	LOGI("INFO(%s): Walle HAL Support preview RES:%dx%d", __func__, mSupportedPreviewSizes[i].width,
    		mSupportedPreviewSizes[i].height);
    }
#else
    // Set preview & picture size supported
    p.set(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES,
          "1280x720,720x480,640x480,320x240,176x144");
    p.set(CameraParameters::KEY_SUPPORTED_PICTURE_SIZES,
          "2592x1944,2048x1536,1600x1200,1280x960,1024x768,640x480,320x240");
    p.set(CameraParameters::KEY_SUPPORTED_VIDEO_SIZES,
    ...
    //Set Preview Size
    p.setPreviewSize(BACK_PREVIEW_WIDTH, BACK_PREVIEW_HEIGHT);
    ...
    p.setPreviewFrameRate(VIDEO_BACK_FRAME_RATE);
    ...
    //focal length
    p.set(CameraParameters::KEY_FOCAL_LENGTH, BACK_CAMERA_FOCAL_LENGTH_STR);

    //ZOOM
    //KEY_ZOOM_SUPPORTED, KEY_ZOOM, KEY_MAX_ZOOM, KEY_ZOOM_RATIOS, KEY_SMOOTH_ZOOM_SUPPORTED
    //We Support 2Times ZOOM and each times up 0.1
    p.set(CameraParameters::KEY_ZOOM_SUPPORTED, "true");
    //p.set(CameraParameters::KEY_MAX_ZOOM, ZOOM_LEVEL_20);//0~20
    p.set(CameraParameters::KEY_MAX_ZOOM, ZOOM_LEVEL_10);//0~10
    p.set(CameraParameters::KEY_ZOOM, ZOOM_LEVEL_0);//
    //NOTE:the zoom func was implenmented by ISP at different resolution
    p.set(CameraParameters::KEY_ZOOM_RATIOS, "100,110,120,130,140,150,160,170,180,190,200");
    ...
    //view field
    p.set(CameraParameters::KEY_HORIZONTAL_VIEW_ANGLE, BACK_CAMERA_HVEIW_ANGLE);
    p.set(CameraParameters::KEY_VERTICAL_VIEW_ANGLE, BACK_CAMERA_VVEIW_ANGLE);
    ...
    // set frame rate into sensor
    mNuCameraV4L2->setFrameRate(VIDEO_BACK_FRAME_RATE);
}

void NuCameraHardware::initDefaultParameters(int cameraId)
{
    ...
    if (cameraId == NuCameraV4L2::CAMERA_ID_BACK){
         initBackParameters();
    } else {
         initFrontParameters();
    }
...
status_t NuCameraHardware::setPreviewWindow(preview_stream_ops *w)
{
	...
	LOGV("%s: set_buffers_geometry(preview_res:%dx%d)", __func__, preview_width, preview_height);
...
int NuCameraHardware::previewThread()
{
   ...
   /* if(!isPreviewTime()){
    ...
    struct timeval begin,end,gr,trans,crop,mcp;
...
#ifdef WALLE_PARAMS
    int stream_w, stream_h, stream_size;
    mNuCameraV4L2->getStreamSize(&stream_w, &stream_h, &stream_size);
#endif
    ...
    //test--for--walle--[
    EWLLinearMem_t pmem2;
    mNuCameraV4L2->getPreviewFrameAddr(NUM_PREVIEW_BUFFERS, &pmem2);
    //test--for--walle--]
#ifdef WALLE_PARAMS
    //LOGD("---test---,mWalleMode:%d", mWalleMode);
    if(mWalleMode != WALLE_AUTO_CALIBRATE){
        //test--for--walle--[
        memcpy(mBayer2NVHeap, pmem.virtualAddress, 960*480);
        //test--for--walle--]
        //gesture and 3Thrd app case:
        //---test---[
        //notify(index);
        //sleep(1);
        //---test---]
        /*
        HandControlProcess((unsigned char *)pmem.virtualAddress,
            960, 480, uinput_report_touch_event, uinput_report_key_event, notify);
        */
        HandControlProcess(mBayer2NVHeap, 960, 480, uinput_report_touch_event, uinput_report_key_event, notify);
#ifdef PERFORMANCE_ESTIMATE
        gettimeofday(&gr,NULL);
        LOGD("---test--- gestureRecognitionPrecess elapse time: %lu ms",
            ((gr.tv_sec * 1000) + (gr.tv_usec/1000)) - ((begin.tv_sec * 1000) + (begin.tv_usec/1000)));
#endif
    } else {
        //TODO:autocalibrate case:
        //test-----[
        if(msgTypeEnabled(CAMERA_MSG_WALLE_AUTO_CALIBRATE_STATE)){
            //LOGD("---test--- auto calibrate,post acState:%d",index);
            //memcpy(mPreviewHeap->data, &index, sizeof(int));
            //mDataCb(CAMERA_MSG_WALLE_AUTO_CALIBRATE_STATE, mPreviewHeap, 0, NULL, mCallbackCookie);
            //sleep(1);
        }
        //test-----]
    }
...
#ifdef  WALLE_PARAMS
    /*
     * NOTE:comment it for 3D team requirement(trans func is too slow!).
     */
     /*
        EWLLinearMem_t pmem2;
        mNuCameraV4L2->getPreviewFrameAddr(NUM_PREVIEW_BUFFERS, &pmem2);
    ...
    //     ((mcp.tv_sec * 1000) + (mcp.tv_usec/1000)) - ((gr.tv_sec * 1000) + (gr.tv_usec/1000)));
        ...
        mNuCameraV4L2->bayer2NV12(mBayer2NVHeap, pmem2.virtualAddress, stream_w, stream_h, mBayerMode);
#ifdef PERFORMANCE_ESTIMATE
...
#endif
        //mNuCameraV4L2->scalingDown420SP(pmem2.virtualAddress, stream_w, stream_h,pmem2.virtualAddress, width, height);
        mNuCameraV4L2->scalingDown420SP_4_walle(pmem2.virtualAddress, stream_w, stream_h,pmem2.virtualAddress, width, height);
#ifdef PERFORMANCE_ESTIMATE
        //Crop Time:
    gettimeofday(&crop,NULL);
    LOGD("---test--- crop elapse time: %lu ms",
         ((crop.tv_sec * 1000) + (crop.tv_usec/1000)) - ((trans.tv_sec * 1000) + (trans.tv_usec/1000)));
#endif
        memcpy(&pmem, &pmem2, sizeof(pmem));
        index = NUM_PREVIEW_BUFFERS;
#endif
        void *gralloc_mem_addr = NULL;
...
callbacks:
    ...
    LOGD("---test--- one frame elapse time: %lu ms",
...
status_t NuCameraHardware::startPreview()
{
    ...
    //zhuyx--add--[
    if(mWalleMode == WALLE_3THRD_GESTURE){
        //only 3Thrd app will use preview window to show frame.
        //we do not init preview window in camera service when only gesture func running.
		if (!mPreviewWindow) {
...
#ifdef WALLE_PARAMS
    mBayerMode = mNuCameraV4L2->getBayerMode();
    mNuCameraV4L2->setStreamSize(GR_NORMAL_WIDTH, GR_NORMAL_HEIGHT);
#endif
...
status_t NuCameraHardware::startPreviewInternal()
{
    //zhuyx--add--walle---[
	/*
    char isWalleRunning[10];
    property_get(CAMERA_WALLE_SRV_RUNNING, isWalleRunning, "0");
    if(strcmp(isWalleRunning, "0") == 0){
    //NOTE:if walle run ,we do not need callback data to upcalls,
    //so there is no need to alloc mem
		if (!mPreviewHeap) {
		    int w, h, size;
	...
	*/
    if(mWalleMode == WALLE_3THRD_GESTURE){
        //NOTE:if walle mode is Gesture/Auto Calib ,we do not need callback data to upcalls,
        //so there is no need to alloc mem
		if (!mPreviewHeap) {
            LOGD("---test----, case WALLE_3THRD_GESTURE, malloc previewHeap..");
		    int w, h, size;
		    mNuCameraV4L2->getPreviewSize(&w, &h, &size);
		    mPreviewHeap = mGetMemoryCb(-1, size, 1, 0);
		    if (!mPreviewHeap) {
...
int NuCameraHardware::pictureThreadInternal()
{
    ...
    int stream_w, stream_h, stream_size;
    mNuCameraV4L2->getStreamSize(&stream_w, &stream_h, &stream_size);
    mNuCameraV4L2->getCaptureFrameAddr(NUM_CAPTURE_BUFFERS, &pmem2);
    //bayer to nv12/nv21 : 1/0
    //and down scaling if necessary
    LOGD("~~~~bayer mode: %d, width=%d, height=%d~~~~", mBayerMode, picWidth, picHeight);
    mNuCameraV4L2->bayer2NV12(pmem.virtualAddress, pmem2.virtualAddress,stream_w, stream_h, mBayerMode);
    //mNuCameraV4L2->scalingDown420SP(pmem2.virtualAddress, stream_w, stream_h, pmem2.virtualAddress, picWidth, picHeight);
    mNuCameraV4L2->scalingDown420SP_4_walle(pmem2.virtualAddress, stream_w, stream_h, pmem2.virtualAddress, picWidth, picHeight);
    memcpy(&pmem, &pmem2, sizeof(pmem));
    index = NUM_CAPTURE_BUFFERS;
#endif
...
{
    LOGI("INFO: sendCommand(cmd:%d,arg1:%d,arg2:%d)", command, arg1, arg2);
    switch(command){
        case CMD_WALLE_MODE:
            {
			    mWalleMode = arg1;
...
void NuCameraHardware::release()
{
    ...
    //zhuyx--add--for-uinput--[
#ifdef WALLE_PARAMS
    LOGI("INFO(%s): Call stopNotifyThread() to stop notify socket.", __func__);
    gNM->stopNotifyThread();
    LOGI("INFO(%s): Call uinput_destroy() to deinit uinput device", __func__);
    uinput_destroy();
    LOGI("INFO(%s), Call RealaseHandControlProcess() to deinit gestureRecognitionPrecess", __func__);
    RealaseHandControlProcess();
#endif
    //zhuyx--add--for-uinput--]
}
                                  ...
                                  struct hw_device_t** device)
{
    ...
    g_cam_device->priv = new NuCameraHardware(sCameraInfo[cameraId].facing, g_cam_device);
