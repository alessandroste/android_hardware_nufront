                                               ...
                                               int effectType)
{
    ...
    const status_t res =
        EmulatedCameraDevice::commonStartDevice(width, height, pix_fmt, 0);
    if (res == NO_ERROR) {
...
void EmulatedQemuCameraDevice::drawNv21Board()
{
    ...
    LOGV("%s: calld drawSquare()", __FUNCTION__);
    drawSquare(sqx * sqsize / 32, sqy * sqsize / 32, (sqsize * 5) >> 1,
               (mCcounter & 0x100) ? &mRedYUV : &mGreenYUV);
    mCcounter++;
}

void EmulatedQemuCameraDevice::drawCheckerboard()
{
     drawNv21Board();
}

void EmulatedQemuCameraDevice::drawSquare(int x,
                                          int y,
                                          int size,
...
void EmulatedQemuCameraDevice::drawSolid(YUVPixel* color)
{
    /* All Ys are the same. */
...
void EmulatedQemuCameraDevice::drawStripes()
{
...
int EmulatedQemuCameraDevice::rotateFrame()
{
