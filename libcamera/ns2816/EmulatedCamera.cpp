...
status_t EmulatedCamera::Initialize()
{
    ...
    mParameters.set(CameraParameters::KEY_SUPPORTED_JPEG_THUMBNAIL_SIZES,
                    "320x240,0x0");
    /*
    //zhuyx --comment it--[we have not supported exposure yet
    ...
    */
    ...
    paramString = CameraParameters::EFFECT_NONE;
    paramString.append(",");
    paramString.append(CameraParameters::EFFECT_MONO);
    paramString.append(",");
    paramString.append(CameraParameters::EFFECT_NEGATIVE);
    paramString.append(",");
    paramString.append(CameraParameters::EFFECT_SOLARIZE);
    paramString.append(",");
    //TODO:SEPIA is not support perfect yet
    //paramString.append(CameraParameters::EFFECT_SEPIA);
    //paramString.append(",");
    paramString.append(CameraParameters::EFFECT_AQUA);
    mParameters.set(CameraParameters::KEY_SUPPORTED_EFFECTS, paramString.string());

    //just for test-[
    /*
    paramString = CameraParameters::WHITE_BALANCE_AUTO;
    paramString.append(",");
    paramString.append(CameraParameters::WHITE_BALANCE_INCANDESCENT);
    paramString.append(",");
    paramString.append(CameraParameters::WHITE_BALANCE_FLUORESCENT);
    paramString.append(",");
    paramString.append(CameraParameters::WHITE_BALANCE_DAYLIGHT);
    paramString.append(",");
    paramString.append(CameraParameters::WHITE_BALANCE_CLOUDY_DAYLIGHT);
    mParameters.set(CameraParameters::KEY_SUPPORTED_WHITE_BALANCE,
          paramString.string());
    mParameters.set(CameraParameters::KEY_WHITE_BALANCE, CameraParameters::WHITE_BALANCE_AUTO);

    //scene mode
    paramString = CameraParameters::SCENE_MODE_AUTO;
    paramString.append(",");
    paramString.append(CameraParameters::SCENE_MODE_PORTRAIT);
    paramString.append(",");
    paramString.append(CameraParameters::SCENE_MODE_LANDSCAPE);
    paramString.append(",");
    paramString.append(CameraParameters::SCENE_MODE_NIGHT);
    paramString.append(",");
    paramString.append(CameraParameters::SCENE_MODE_BEACH);
    paramString.append(",");
    paramString.append(CameraParameters::SCENE_MODE_SNOW);
    paramString.append(",");
    paramString.append(CameraParameters::SCENE_MODE_SUNSET);
    paramString.append(",");
    paramString.append(CameraParameters::SCENE_MODE_FIREWORKS);
    paramString.append(",");
    paramString.append(CameraParameters::SCENE_MODE_SPORTS);
    paramString.append(",");
    paramString.append(CameraParameters::SCENE_MODE_PARTY);
    paramString.append(",");
    paramString.append(CameraParameters::SCENE_MODE_CANDLELIGHT);
...
status_t EmulatedCamera::startRecording()
{
    ...
    return -mCallbackNotifier.enableVideoRecording(mParameters.getPreviewFrameRate(),camera_dev);
}
...
status_t EmulatedCamera::takePicture()
{
    ...
    //Camera Effect Support
    const char* effect = NULL;
    int effectType;
    effect = mParameters.get(CameraParameters::KEY_EFFECT);
    if (strcmp(effect, CameraParameters::EFFECT_NONE) == 0) {
        effectType = Effect::CAMERA_EFFECT_NONE;
        LOGV("EmulatedCamera::doStartPreview,case CAMERA_EFFECT_NONE");
    } else if (strcmp(effect, CameraParameters::EFFECT_MONO) == 0) {
        effectType = Effect::CAMERA_EFFECT_MONO;
        LOGV("EmulatedCamera::doStartPreview,case CAMERA_EFFECT_MONO");
    } else if (strcmp(effect, CameraParameters::EFFECT_NEGATIVE) == 0) {
        effectType = Effect::CAMERA_EFFECT_NEGATIVE;
        LOGV("EmulatedCamera::doStartPreview,case CAMERA_EFFECT_NEGATIVE");
    } else if (strcmp(effect, CameraParameters::EFFECT_SOLARIZE) == 0) {
        effectType = Effect::CAMERA_EFFECT_SOLARIZE;
        LOGV("EmulatedCamera::doStartPreview,case CAMERA_EFFECT_SOLARIZE");
    } else if (strcmp(effect, CameraParameters::EFFECT_AQUA) == 0) {
        effectType = Effect::CAMERA_EFFECT_AQUA;
        LOGV("EmulatedCamera::doStartPreview,case CAMERA_EFFECT_AQUA");
    } else {
        effectType = Effect::CAMERA_EFFECT_NONE;
        LOGV("EmulatedCamera::doStartPreview,case CAMERA_EFFECT_NONE");
    }

    LOGD("Starting camera: %dx%d -> %.4s(%s) ,effect:%d",
         width, height, reinterpret_cast<const char*>(&org_fmt), pix_fmt, effectType);
    /*
    ...
    res = camera_dev->startDevice(width, height, org_fmt, effectType);
    if (res != NO_ERROR) {
        if (preview_on) {
            doStartPreview();
        }
...
status_t EmulatedCamera::setParameters(const char* parms)
{
    ...
    int effctType;
    //effect = mParameters.get(CameraParameters::KEY_EFFECT);
    effect = new_param.get(CameraParameters::KEY_EFFECT);
    if (strcmp(effect, CameraParameters::EFFECT_NONE) == 0) {
        effctType = Effect::CAMERA_EFFECT_NONE;
        LOGV("EmulatedCamera::setParameters,case CAMERA_EFFECT_NONE");
    } else if (strcmp(effect, CameraParameters::EFFECT_MONO) == 0) {
        effctType = Effect::CAMERA_EFFECT_MONO;
        LOGV("EmulatedCamera::setParameters,case CAMERA_EFFECT_MONO");
    } else if (strcmp(effect, CameraParameters::EFFECT_NEGATIVE) == 0) {
        effctType = Effect::CAMERA_EFFECT_NEGATIVE;
        LOGV("EmulatedCamera::setParameters,case CAMERA_EFFECT_NEGATIVE");
    } else if (strcmp(effect, CameraParameters::EFFECT_SOLARIZE) == 0) {
        effctType = Effect::CAMERA_EFFECT_SOLARIZE;
        LOGV("EmulatedCamera::setParameters,case CAMERA_EFFECT_SOLARIZE");
    } else if (strcmp(effect, CameraParameters::EFFECT_AQUA) == 0) {
        effctType = Effect::CAMERA_EFFECT_AQUA;
        LOGV("EmulatedCamera::setParameters,case CAMERA_EFFECT_AQUA");
    } else {
        effctType = Effect::CAMERA_EFFECT_NONE;
        LOGV("EmulatedCamera::setParameters,case CAMERA_EFFECT_NONE");
    }
    LOGV("EmulatedCamera::setParameters,camera_dev->setColorEffect(effctType:%d", effctType);
    camera_dev->setColorEffect(effctType);

    // rotation
    int new_rotation = new_param.getInt(CameraParameters::KEY_ROTATION);
    LOGV("%s : new_rotation %d", __func__, new_rotation);
    if (0 <= new_rotation) {
        LOGV("%s : set orientation:%d\n", __func__, new_rotation);
        if (camera_dev->setExifOrientationInfo(new_rotation) < 0) {
            LOGE("ERR(%s):Fail on camera_dev->setExifOrientationInfo(%d)", __func__, new_rotation);
        }
    }
...
status_t EmulatedCamera::doStartPreview()
{
    ...
    LOGD("Starting camera: %dx%d -> %.4s(%s) ,effect:%d",
         width, height, reinterpret_cast<const char*>(&org_fmt), pix_fmt, effctType);
    res = camera_dev->startDevice(width, height, org_fmt, effctType);
    if (res != NO_ERROR) {
        mPreviewWindow.stopPreview();
        return res;
    }
