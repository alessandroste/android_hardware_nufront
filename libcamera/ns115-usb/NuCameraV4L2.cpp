...
#if 0
#define JCG(frm,args...)      LOGE("--James--[%s:%d]--<%s>--"frm"\n",__FILE__,__LINE__,__func__,##args);
#else
#define JCG(frm,args...)
#endif

static int usbPreviewSize[][2] = {
		{1280,720},
		{640,480},
		{800,600},
		{320,240},
	...
	static void yuyv422ToYuv420(void* src, void* dst, int width, int height) {
		uint32_t *Y = (uint32_t *)dst;
		uint32_t *UV = (uint32_t *)((uint8_t *)dst + width * height);
		uint32_t *Y2, *src_pos2;
		uint32_t *src_pos = (uint32_t *)src;
		uint32_t data, data1, data2, data3, tmp_uv2, tmp_uv;
		int x, y;
	
		for (y = 0; y < height; y += 2) {
			Y2 = (uint32_t *)((uint8_t *)Y + width);
			src_pos2 = (uint32_t *)((uint8_t *)src_pos + width * 2);
			for (x = 0; x < width; x += 4) {
				data = *src_pos++;
				data1 = *src_pos++;
				data2 = *src_pos2++;
				...
				*Y++ = (data & 0xFF) + ((data & 0xFF0000) >> 8) + ((data1 & 0xFF) << 16) + ((data1 & 0xFF0000) << 8);
				*Y2++ = (data2 & 0xFF) + ((data2 & 0xFF0000) >> 8) + ((data3 & 0xFF) << 16) + ((data3 & 0xFF0000) << 8);
				tmp_uv = ((data & 0xFF00) >> 8) + ((data & 0xFF000000) >> 16) + ((data1 & 0xFF00) << 8) + (data1 & 0xFF000000);
				tmp_uv2 = ((data2 & 0xFF00) >> 8) + ((data2 & 0xFF000000) >> 16) + ((data3 & 0xFF00) << 8) + (data3 & 0xFF000000);
				__asm__ __volatile__ ("UHADD8 %0, %0, %1":"+r"(tmp_uv2):"r"(tmp_uv));
				*UV++ = tmp_uv2;
			}
			Y = Y2;
			src_pos = src_pos2;
...
//static unsigned int usbspport_fmt = 0;
...
static void nusmart_scalingDown420SP(void*src, int srcWidth, int srcHeight,
...
int NuCameraV4L2::nusmart_v4l2_querycap(int fp)
{
	LOGV("%s:", __func__);
	char DriverName[16];
	...
	LOGV("driver name: (%s) .", cap.driver);

	strcpy(DriverName,(char*)cap.driver);
	if(!strcmp(DriverName,"uvcvideo")) {
		LOGV("USB camera!", DriverName);
		camera->isUsbCamera = true;
		camera->isCameraFaceBack = true;//USB camera as back camera
	}else {
			LOGV("NO USB camera!", DriverName);
			camera->isUsbCamera = false;
			camera->isCameraFaceBack = false;
	 }
...
int NuCameraV4L2::nusmart_v4l2_enum_reso()	
{
	int ret = 0;
	struct v4l2_frmsizeenum v4l2_frmesize;	
	int TempReso = 0;
    int Reso = 0;
	v4l2_frmesize.pixel_format = V4L2_PIX_FMT_YUYV;
	for (int i =0 ; ; i++) { 		
		v4l2_frmesize.index = i;
		ret = ioctl(m_cam_fd, VIDIOC_ENUM_FRAMESIZES, &v4l2_frmesize);
		if (ret < 0) {
			break;
		}
		if (V4L2_FRMSIZE_TYPE_STEPWISE == v4l2_frmesize.type) {
			LOGV("min_width = %d",v4l2_frmesize.stepwise.min_width);		
			LOGV("max_width = %d",v4l2_frmesize.stepwise.max_width);
			LOGV("step_width = %d",v4l2_frmesize.stepwise.step_width);
			LOGV("min_height = %d",v4l2_frmesize.stepwise.min_height);
			LOGV("max_height = %d",v4l2_frmesize.stepwise.max_height);
			LOGV("step_height = %d",v4l2_frmesize.stepwise.step_height);
			m_preview_max_width = v4l2_frmesize.stepwise.max_width;
			m_preview_max_height = v4l2_frmesize.stepwise.max_height;
			m_snapshot_max_width = v4l2_frmesize.stepwise.max_width;
			m_snapshot_max_height = v4l2_frmesize.stepwise.max_height;
		} else if (V4L2_FRMSIZE_TYPE_DISCRETE == v4l2_frmesize.type) {
			TempReso = v4l2_frmesize.discrete.width * v4l2_frmesize.discrete.height;
			if (TempReso > Reso) {
				Reso = TempReso;
				m_preview_max_width = v4l2_frmesize.discrete.width;
				m_preview_max_height = v4l2_frmesize.discrete.height;
				m_snapshot_max_width = v4l2_frmesize.discrete.width;
				m_snapshot_max_height = v4l2_frmesize.discrete.height;
			}
			LOGV("resolution%d:(%dx%d)", i, v4l2_frmesize.discrete.width, v4l2_frmesize.discrete.height);
		 } else {
			LOGV("BOTH NOT SUPPORT!");
		  }

		if (i == 0 && V4L2_FRMSIZE_TYPE_DISCRETE == v4l2_frmesize.type) {
			if (!camera->isUsbCamera) {
			 	 camera->mPreviewSize.width = v4l2_frmesize.discrete.width;;
			 	 camera->mPreviewSize.height = v4l2_frmesize.discrete.height;					
			   }
		    sprintf(camera->supportPreviewSize,"%dx%d",v4l2_frmesize.discrete.width,v4l2_frmesize.discrete.height);
		} else {
		    if (i == 1 && !camera->isUsbCamera) 
			    sprintf(camera->mVideoSize,"%dx%d",v4l2_frmesize.discrete.width,v4l2_frmesize.discrete.height);
		    sprintf(camera->supportPreviewSize,"%s,%dx%d",camera->supportPreviewSize,v4l2_frmesize.discrete.width,v4l2_frmesize.discrete.height);
		}
	}	

	if (camera->isUsbCamera) {
	    CameraParameters p;
		p.set(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES, camera->supportPreviewSize);
	    p.getSupportedPreviewSizes(camera->mSupportedPreviewSizes);
		
		for (int i=0 ;i<(sizeof(usbPreviewSize)/sizeof(usbPreviewSize[0])) ;i++) {
			camera->mPreviewSize.width = usbPreviewSize[i][0];
			camera->mPreviewSize.height = usbPreviewSize[i][1];
			sprintf(camera->mVideoSize,"%dx%d",usbPreviewSize[i][0],usbPreviewSize[i][1]);
			if (camera->isSupportedPreviewSize(camera->mPreviewSize.width, camera->mPreviewSize.height)) {
				LOGV("found usb camera mVideoSize. camera->mVideoSize(%s)", camera->mVideoSize);
				break;
			}
		}
	}
	
	LOGV("mPreviewSize(%dx%d). supportPreviewSize: %s",camera->mPreviewSize.width, camera->mPreviewSize.height, camera->supportPreviewSize);
	return 0;
}
...
int NuCameraV4L2::nusmart_v4l2_s_fmt(int fp, unsigned int width, unsigned int height, unsigned int fmt, int flag_capture)
{
    ...
    if(base_width != width || base_height != height && !camera->isUsbCamera){
		...
		do{
			...
			if (ret < 0) {
	            LOGE("jzyang!!!ERR(%s):VIDIOC_S_FMT failed. %s(%d)\n", __func__, strerror(errno), errno);
				return -1;
	        }
		}while(0);//ret != 0);
...
static int enum_frame_formats(int dev, u32 *supported_formats, unsigned int max_formats)
{
	int ret;
    ...
    while(ioctl(dev, VIDIOC_ENUM_FMT, &fmt) == 0) {
		  ...
		  LOGV("support pixelformat:(%d)", fmt.pixelformat);
		  fmt.index++;
	}
...
int NuCameraV4L2::USB_v4l2_enum_fmt(int fp, unsigned int fmt)
{
	// Assume no device supports more than 16 formats
	u32 device_formats[16] = { 0 };	
	int requested_format_found = 0, fallback_format = -1;
	
	if(enum_frame_formats(fp, device_formats, ARRAY_SIZE(device_formats))) {
 	   LOGV("Unable to enumerate frame formats");
 	   return -1;
	}
	
	for( int i = 0; i < ARRAY_SIZE(device_formats) && device_formats[i]; i++) {
		if(device_formats[i] == fmt) {
 		   requested_format_found = 1;
 		   break;
		}
		if(device_formats[i] == V4L2_PIX_FMT_YUYV)
		   fallback_format = i;
	}
	
	if(requested_format_found) {
	    LOGV("The requested format is supported");
    }else if(fallback_format >= 0) {
	    LOGV("The requested format(%d) is not supported but there's a fallback format", fmt);
	    CameraSupportFmt = device_formats[fallback_format];
	    if(CameraSupportFmt == V4L2_PIX_FMT_YUYV) {
			LOGV("usb camera support V4L2_PIX_FMT_YUYV(%d)",CameraSupportFmt);
	    }
	 }else {
 	    LOGV("The requested format is not supported and no fallback format is available");
	    return -1;
	  }
...
static int nusmart_v4l2_reqbufs(int fp, enum v4l2_buf_type type, int nr_bufs)
{
	...
	for(i=0; i<10; i++)
	{
    	ret = ioctl(fp, VIDIOC_REQBUFS, &req);
            if (ret < 0) 
				{
        			LOGE("ERR(%s):Cannot reqbufs", __func__);
        			continue;
    			}
    			break;
	}
...
static int USB_v4l2_reqbufs(int fp, enum v4l2_buf_type type, int nr_bufs)
{
    struct v4l2_requestbuffers req;
    int ret;
	//LOGV("%s" , __func__);
...
int USB_v4l2_qbuf(int fp, int index)
{
    struct v4l2_buffer v4l2_buf;
    memset(&v4l2_buf, 0, sizeof(struct v4l2_buffer));
    int ret;
...
int NuCameraV4L2::nusmart_v4l2_querybuf(int fp,  enum v4l2_buf_type type, int index)
{
    struct v4l2_buffer v4l2_buf;  
	memset(&v4l2_buf, 0, sizeof(struct v4l2_buffer));
    int ret;
    ...
    if ((tempbuff[index].start = (char *)mmap(0, v4l2_buf.length,
                                         PROT_READ | PROT_WRITE, MAP_SHARED,
                                         fp, v4l2_buf.m.offset)) < 0) {
         LOGE("%s %d] mmap() failed\n",__func__, __LINE__);
         return -1;
    }
	tempbuff[index].length = v4l2_buf.length;

    LOGD("%s: addr = 0x%08x, v4l2_buf.length = %d",
         __func__,NuCameraV4L2::tempbuff[index].start, v4l2_buf.length);
...
int NuCameraV4L2::nusmart_v4l2_enum_ctl_id(int fd) 
{
	LOGV("%s", __func__);
	int ret = 0;
	
	for(int i = V4L2_CID_BRIGHTNESS; i<=V4L2_CID_VCENTER; i++) {
		struct v4l2_queryctrl quryctrl;
		quryctrl.id = i;
		if(i == V4L2_CID_VCENTER){
				static int j = 1;//we can add for more for mrv_camera
				if(j) {
				   quryctrl.id = V4L2_CID_CAMERA_SPECIAL_EFFECT;
				   j--;
				   i--;
				} else {
					quryctrl.id = V4L2_CID_CAMERA_FACING;
					j = 1;//must init to 1!
				}
			}
		ret = ioctl(fd, VIDIOC_QUERYCTRL, &quryctrl);
		if (ret < 0) {
			continue;
		} else {
				//LOGV("support id = %d", i);
				if (quryctrl.type == V4L2_CTRL_TYPE_INTEGER) {
					if ( V4L2_CID_BRIGHTNESS == quryctrl.id) {
						/*  do not delete, will use later maybe
						sprintf(exposure_compensation,"%d",quryctrl.default_value);
						sprintf(max_exposure_compensation,"%d",quryctrl.maximum);				
						sprintf(min_exposure_compensation,"%d",quryctrl.minimum);		
						sprintf(exposure_compensation_step,"%d",quryctrl.step);
						*/
						camera->isSupportSetBrightness = true;
						camera->exposure = quryctrl.default_value;
						camera->max_exposure = quryctrl.maximum;
						camera->min_exposure = quryctrl.minimum;
						camera->exposure_step = quryctrl.step;
					}
					if (V4L2_CID_CAMERA_FACING==quryctrl.id) {
						if(quryctrl.default_value);
							/*we can make  default_value indecate it back or front camera
							camera->isCameraFaceBack = true;*/
					}
										
					LOGV("interger: name=%s, minimum=%d, maximum=%d, step=%d, default_value=%d", 
						quryctrl.name, quryctrl.minimum, quryctrl.maximum, quryctrl.step, quryctrl.default_value);
					/*add more later*/
				}else if(quryctrl.type == V4L2_CTRL_TYPE_BOOLEAN) {			
				  	if (V4L2_CID_VFLIP == quryctrl.id) {
						camera->isSupportVflip = true;
						camera->default_Vflip = quryctrl.default_value;
				    }
				  	if (V4L2_CID_HFLIP == quryctrl.id) {
						camera->isSupportHflip = true;
						camera->default_Hflip = quryctrl.default_value;
				    }
				  	if (V4L2_CID_AUTO_WHITE_BALANCE == quryctrl.id) {
						camera->isSupportAutoWhiteBalance= true;
						camera->default_AutoWhiteBalance = quryctrl.default_value;
						camera->min_AutoWhiteBalance  = quryctrl.minimum;
						camera->max_AutoWhiteBalance  = quryctrl.maximum;
						camera->AutoWhiteBalance_step = quryctrl.step;
						tempWhite = quryctrl.step;
				    }

					LOGV("bollean: name=%s, minimum=%d, maximum=%d, step=%d, default_value=%d", 
				         quryctrl.name, quryctrl.minimum, quryctrl.maximum, quryctrl.step, quryctrl.default_value);
				/*add more later*/
				 }else if(quryctrl.type == V4L2_CTRL_TYPE_MENU) {
				  	if (V4L2_CID_DO_WHITE_BALANCE==quryctrl.id) {
						camera->isSupportDoWhiteBalance = true;
						camera->default_light_mode = quryctrl.default_value;
						camera->max_light_mode = quryctrl.maximum;
						camera->min_light_mode = quryctrl.minimum;
						camera->light_mode_step = quryctrl.step;
				    }
					/*V4L2_CID_CAMERA_SPECIAL_EFFECT is not std V4L2*/
				  	if (V4L2_CID_CAMERA_SPECIAL_EFFECT==quryctrl.id) {
						camera->isSupportSpecialEffect = true;
						camera->default_special_effect = quryctrl.default_value;
						camera->max_special_effect = quryctrl.maximum;
						camera->min_special_effect = quryctrl.minimum;
						camera->special_mode_effect = quryctrl.step;
				    }								
					/*add more later*/
					LOGV("menu: name=%s, minimum=%d, maximum=%d, step=%d, default_value=%d", 
			             quryctrl.name, quryctrl.minimum, quryctrl.maximum, quryctrl.step, quryctrl.default_value);
				 }			
			}
	}
...
static int is_usb_camera(int fp)
{
	char DriverName[16];
    struct v4l2_capability cap;
    int ret = 0;
...
int NuCameraV4L2::v4l2getNumberOfCameras(bool *isUsbOneCamera)
{
	int number = 0;
	int fd = -1;
	int n = 4;
	...
	do{
    	...
    	if (fd < 0) {  
    	   ...
    	   number++;
		   if (1 == is_usb_camera(fd)) 
		   	   *isUsbOneCamera = true;
		   else 
		   	   *isUsbOneCamera = false;
		   close(fd);
		   fd = -1;
		   n = 4;
		   break;
	...
	LOGV("number(%d), isUsbOneCamera(%d)", number , *isUsbOneCamera);    
	return number;
}
...
NuCameraV4L2::NuCameraV4L2() :
            ...
            m_preview_width      (0),
            m_preview_height     (0),
            m_preview_max_width  (0),//MAX_BACK_CAMERA_PREVIEW_WIDTH
            m_preview_max_height (0),//MAX_BACK_CAMERA_PREVIEW_HEIGHT
            m_snapshot_v4lformat(-1),
            m_snapshot_width      (0),
            m_snapshot_height     (0),
            m_snapshot_max_width  (0),//MAX_BACK_CAMERA_SNAPSHOT_WIDTH
            m_snapshot_max_height (0),//MAX_BACK_CAMERA_SNAPSHOT_HEIGHT
            m_angle(-1),
            m_anti_banding(-1),
            m_wdr(-1),
            m_anti_shake(-1),
...
int NuCameraV4L2::initCamera(NuCameraHardware* thiscamera, int index, int size)
{
    ...
    if (!m_flag_init) {
		...
		nusmart_v4l2_enum_reso();
		nusmart_v4l2_enum_ctl_id(m_cam_fd);
...
int NuCameraV4L2::openCameraDevice(int camera_facing)
{
    ...
    ret = nusmart_v4l2_querycap(fd);
    CHECK(ret);
	
    if (camera->isUsbCamera){//!UsbCamera     make the usb camera became back camera
        m_camera_back_dev = 0;
    } else if (!camera->isUsbCamera){  //UsbCamera     make the usb camera became back camera
        m_camera_back_dev = 1;
    } else {
        LOGE("ERR(%s): error value", __func__);
        return -1;
    }
...
void NuCameraV4L2::resetCamera()
{
    LOGV("%s :", __func__);
    DeinitCamera();
    initCamera(camera, m_camera_id, 0);
}
...
void NuCameraV4L2::DeinitCamera()
{
	...
	//camera = NULL;   do not make camera = NULL, it will used is initCamera!
	tempWhite = 0;
	CameraSupportFmt = 0;
}
...
int NuCameraV4L2::startPreview(void)
{
	...
	if(!camera->isUsbCamera) {
		...
		// enum_fmt  for USB camera
		ret = USB_v4l2_enum_fmt(m_cam_fd,m_preview_v4lformat);
		CHECK(ret);
	  }
	...
	if(camera->isUsbCamera) {
		ret = USB_v4l2_reqbufs(m_cam_fd, V4L2_BUF_TYPE_VIDEO_CAPTURE, NUM_PREVIEW_BUFFERS);
		CHECK(ret);
	} else {
	...
	if(camera->isUsbCamera){
		for( int i =0; i<NUM_PREVIEW_BUFFERS ; i++ ) {
				ret = nusmart_v4l2_querybuf(m_cam_fd, V4L2_BUF_TYPE_VIDEO_CAPTURE, i);
				CHECK(ret);
				if( i>0 ){
...
int NuCameraV4L2::stopPreview(void)
{
	...
	// munmap yuyv tempbuff
	if(camera->isUsbCamera) {
		for (int i=0; i<NUM_PREVIEW_BUFFERS; i++) {
			if (tempbuff[i].start) {
				munmap(tempbuff[i].start, tempbuff[i].length);
				tempbuff[i].start = NULL;
				tempbuff[i].length = 0;
			}
		}
	//	resetCamera();  must reset for usb camera 
	//   modify usbcamera driver for don't have to resetcamera.
	}
...
int NuCameraV4L2::getPreviewFrame()
{
	...
	if (camera->isUsbCamera) {
		//dqbuf the buffer
	    index = USB_v4l2_dqbuf(m_cam_fd);
	    if (!(0 <= index && index < NUM_PREVIEW_BUFFERS)) {
	        LOGE("ERR(%s):wrong index = %d\n", __func__, index);
	        return -1;
	    }
		//usb camera support V4L2_PIX_FMT_YUYV
		if (CameraSupportFmt ==  V4L2_PIX_FMT_YUYV) {
			//struct timeval t, tv2;
	        //t.tv_sec = t.tv_usec = 0;
	        //gettimeofday(&t, NULL);
			yuyv422ToYuv420(tempbuff[index].start, m_ewl_mem_info[index].virtualAddress, m_preview_width, m_preview_height);
			//gettimeofday(&tv2, NULL);
			//LOGI("-------> width = %d, height = %d, time = %u", m_preview_width, m_preview_height, (tv2.tv_sec - t.tv_sec) * 1000000 + tv2.tv_usec - t.tv_usec );
		}
	  	//qbuf the buffer
		USB_v4l2_qbuf(m_cam_fd, index);
	} else {
		int buffer_done;
...
int NuCameraV4L2::setSnapshotCmd(void)
{
    ...
    ret = USB_v4l2_enum_fmt(m_cam_fd,m_snapshot_v4lformat);
    CHECK(ret);

	ret = nusmart_v4l2_s_fmt(m_cam_fd, m_snapshot_width, m_snapshot_height, m_snapshot_v4lformat, 0);
    CHECK(ret);
	...
	if(!camera->isUsbCamera) {
		...
		ret = USB_v4l2_reqbufs(m_cam_fd, V4L2_BUF_TYPE_VIDEO_CAPTURE, NUM_CAPTURE_BUFFERS);
    	CHECK(ret);
	}
	...
	for( int i =0; i<NUM_CAPTURE_BUFFERS ; i++ ) {
			ret = nusmart_v4l2_querybuf(m_cam_fd,V4L2_BUF_TYPE_VIDEO_CAPTURE,i);
			CHECK(ret);
			LOGV("tempbuff[%d].start=0x%08x", i,tempbuff[i].start);
			if( i>0 ) {
...
int NuCameraV4L2::endSnapshot(void)
{
    ...
    ret = nusmart_v4l2_streamoff(m_cam_fd);// why? for what?
    CHECK(ret);
	...
	if(camera->isUsbCamera) {
		...
		//resetCamera();// must reset for usb camer
	}
...
int NuCameraV4L2::getSnapshotFrame()
{
	...
	if(camera->isUsbCamera) {
	    if(ret > 0){
			...
			if( CameraSupportFmt ==  V4L2_PIX_FMT_YUYV ) {//m_snapshot_v4lformat == V4L2_PIX_FMT_NV12
				yuyv422ToYuv420(tempbuff[index].start, m_ewl_capture_mem_info[index].virtualAddress, m_snapshot_width, m_snapshot_height);
			}
			int ret = USB_v4l2_qbuf(m_cam_fd, index);
	        if(ret < 0){
...
/*
void NuCameraV4L2::getBrightnessAddr(char *ex, char *max,char *min,char *step)
{
	LOGV("%s", __func__);
	strcpy(ex,exposure_compensation);
	strcpy(max,max_exposure_compensation);
	strcpy(min,min_exposure_compensation);
	strcpy(step,exposure_compensation_step);
	LOGV("%s, %s, %s, %s", ex, max, min, step);
}
*/
...
int NuCameraV4L2::setFrameRate(int frame_rate)
{
    ...
    if (m_params->capture.timeperframe.denominator != (unsigned)frame_rate) {
        ...
        if (nusmart_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_FRAME_RATE, frame_rate) < 0) {
            ...
            //return -1;    do not return any error!
        }
    }
...
int NuCameraV4L2::setWhiteBalance(int white_balance)
{
	...
	LOGV("isSupportDoWhiteBalance(%d),isSupportAutoWhiteBalance(%d)",camera->isSupportDoWhiteBalance,camera->isSupportAutoWhiteBalance );
	if(!camera->isSupportDoWhiteBalance && !camera->isSupportAutoWhiteBalance) {
		LOGV("Do support to setWhiteBalance!");
        return 0;
	}
	...
	if(camera->isSupportAutoWhiteBalance) {
		if(camera->tempWhileBalance != white_balance) {
			camera->tempWhileBalance = white_balance;
			tempWhite = tempWhite +camera->AutoWhiteBalance_step;
			if(tempWhite > camera->max_AutoWhiteBalance)
			   tempWhite = camera->min_AutoWhiteBalance;
	        if (nusmart_v4l2_s_ctrl(m_cam_fd, V4L2_CID_AUTO_WHITE_BALANCE, tempWhite) < 0) {
				LOGE("ERR(%s):Fail on V4L2_CID_AUTO_WHITE_BALANCE", __func__);
		        return -1;
		    }
		}
	}else {
...
int NuCameraV4L2::setBrightness(int brightness)
{
	...
	if(0 == brightness) {
	   TempBrightness = camera->exposure;
	}else if(4 == brightness) {
			 TempBrightness = camera->max_exposure;
	 }else if(-4 == brightness) {
			  TempBrightness = camera->min_exposure;
	  }else {	
	  		 if(brightness > 0) {
				if(0 == ((camera->max_exposure-camera->exposure)/4)%camera->exposure_step) {
					TempBrightness = (brightness)*((camera->max_exposure-camera->exposure)/4)+camera->exposure;	
				}else {
					TempBrightness = (brightness)*((camera->max_exposure-camera->exposure)/4 - 
						((camera->max_exposure-camera->exposure)/4)%camera->exposure_step)+camera->exposure;	
				 }
	  		 }else {
				 if(0 == ((camera->exposure-camera->min_exposure)/4)%camera->exposure_step) {
					 TempBrightness = (brightness)*((camera->exposure-camera->min_exposure)/4)+camera->exposure;	 
				 }else {
					 TempBrightness = (brightness)*((camera->exposure-camera->min_exposure)/4 - 
						 ((camera->exposure-camera->min_exposure)/4)%camera->exposure_step)+camera->exposure;	 
				  }
	  		  }
	   }

	if (m_params->brightness != brightness || camera->isUsbCamera) {
        m_params->brightness = brightness;
        if (nusmart_v4l2_s_ctrl(m_cam_fd, V4L2_CID_BRIGHTNESS, TempBrightness) < 0) {
...
int NuCameraV4L2::setImageEffect(int image_effect){
    ...
    if (m_params->effects != image_effect) {
        ...
        if (nusmart_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_SPECIAL_EFFECT, image_effect) < 0) {
			if(camera->isSupportSpecialEffect) {
...
#ifdef NU_FLASH_CTRL_IOCTL
#define  NU_FLASH_ON   0x1
#define  NU_FLASH_ONE_TIME   0x00010001
#define  NU_FLASH_OFF  0x0
         ...
         switch (m_params->flash_mode)
         {
             case FLASH_MODE_AUTO:
                 ...
                 if(m_current_aec_value > AUTO_FLASH_THRESHOLD) {
                     ret = nusmart_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_FLASH_MODE, NU_FLASH_ONE_TIME);
                     mFlashFired = true;
                 }
                 break;
             case FLASH_MODE_ON:
             ...
             case FLASH_MODE_TORCH:
                 ret = nusmart_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_FLASH_MODE, NU_FLASH_ON);
                 break;
         }
    } else {
         ret = nusmart_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_FLASH_MODE, NU_FLASH_OFF);
    }
...
void   NuCameraV4L2::getPreviewFrameAddr(int index, EWLLinearMem_t *paddr) const
{
	  ...
	  // LOGV("m_ewl_mem_info[%d].virtualAddress=0x%08x", index ,m_ewl_mem_info[index].virtualAddress);
...
int NuCameraV4L2::thumbnailEncoding(int index)
{
    ...
    nusmart_scalingDown420SP(pYuvIn, m_snapshot_width, m_snapshot_height,
