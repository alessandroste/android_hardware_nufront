...
/* Converts YUV color to RGB32. */
static __inline__ uint32_t
YUVToRGB32WithEffect(int y, int u, int v, int effectType)
{
    /* Calculate C, D, and E values for the optimized macro. */
    y -= 16; u -= 128; v -= 128;
    RGB32_t rgb;
    ...
    if(effectType != Effect::CAMERA_EFFECT_NONE){
        Effect::processEffectPerPixel(&rgb.r,&rgb.g,&rgb.b,effectType);
    }
    return rgb.color;
}
...
/* Converts YUV color to RGB32. */
static __inline__ void
YUVEffectSwitch(uint8_t *y, uint8_t *u, uint8_t *v, int effectType)
{
    /* Calculate C, D, and E values for the optimized macro. */
    (*y) -= 16; (*u) -= 128; (*v) -= 128;
    RGB32_t rgb;
    ...
    rgb.b = YUV2BO((*y),(*u),(*v)) & 0xff;
    /*
    rgb.r = YUV2R((*y),(*u),(*v)) & 0xff;
    rgb.g = YUV2G((*y),(*u),(*v)) & 0xff;
    rgb.b = YUV2B((*y),(*u),(*v)) & 0xff;
    */
    if(effectType != Effect::CAMERA_EFFECT_NONE){
        Effect::processEffectPerPixel(&rgb.r,&rgb.g,&rgb.b,effectType);
    }
    //RGB32ToYUV(rgb,y,u,v);
    //R8G8B8ToYUV(uint8_t r, uint8_t g, uint8_t b, uint8_t* y, uint8_t* u, uint8_t* v)
    RGB32ToYUV(rgb.color, y, u, v);
    //return rgb.color;
}
