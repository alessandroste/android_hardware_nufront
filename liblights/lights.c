...
static int g_fb_blank = FB_BLANK_UNBLANK;
...
char const*const LCD_FILE
        = "/sys/class/backlight/ns-backlight/brightness";

char const*const FB_FILE
        = "/dev/graphics/fb0";
...
write_int(char const* path, int value)
{
    ...
    //ALOGD("Light open write int path:%s value:%d\n",path ,value);
    fd = open(path, O_RDWR);
    if (fd >= 0) {
        char buffer[20];
        ...
        struct light_state_t const* state)
{
    ...
    //ALOGD("Light set_lights_backlight %d\n", brightness);
    pthread_mutex_lock(&g_lock);
    g_backlight = brightness;
    err = write_int(LCD_FILE, (brightness/25));

//@LWC delete this code because of it affects lcdc driver
/*
    if(((brightness == 0) && (g_fb_blank == FB_BLANK_UNBLANK))||
		    ((brightness != 0) && (g_fb_blank != FB_BLANK_UNBLANK)))
    {
	    int fd = open(FB_FILE, O_RDWR);
	    if(fd < 0) {
		    ALOGE("backlight open fb failed.");
	    } else {
		    g_fb_blank = (brightness==0?FB_BLANK_POWERDOWN:FB_BLANK_UNBLANK);
		    ioctl(fd, FBIOBLANK, g_fb_blank);
            close(fd);
	    }
    }
*/
